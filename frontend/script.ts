...
const form = document.getElementById("cep-form") as HTMLFormElement;
const input = document.getElementById("cep-input") as HTMLInputElement;
const submitButton = document.getElementById("cep-submit") as HTMLButtonElement;
const feedback = document.getElementById("cep-feedback") as HTMLParagraphElement;
...
function setFeedback(mensagem: string, estado?: "loading" | "erro"): void {
...
function renderizarShoppings(dados: ShoppingsResponse): void {
  limparResultados();
...
async function buscarShoppings(cep: string): Promise<void> {
  ...
  try {
    ...
    setFeedback(
      'Não foi possível conectar à API em ${API_BASE_URL}. Verifique se o backend Flask está rodando.',
      "erro"
    );
  ...
  let corpo: ShoppingsResponse | ApiError;
  try {
